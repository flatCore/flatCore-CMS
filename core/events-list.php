...
foreach ($get_events as $k => $post) {
    ...
    $get_events[$k]['event_title'] = $get_events[$k]['post_title'];
    $get_events[$k]['event_teaser'] = htmlspecialchars_decode($get_events[$k]['post_teaser']);
    $get_events[$k]['event_text'] = htmlspecialchars_decode($get_events[$k]['post_text']);
    $get_events[$k]['event_id'] = $get_events[$k]['post_id'];

    /* post images */
    $first_post_image = '';
    $post_images = explode("<->", $get_events[$k]['post_images']);
    if ($post_images[1] != "") {
        ...
        $get_events[$k]['event_img_src'] = "/$img_path/" . $fc_prefs['prefs_posts_default_banner'];
    }

    $post_filename = basename($get_events[$k]['post_slug']);
    $get_events[$k]['event_href'] = FC_INC_DIR . "/" . $target_page[0] . "$post_filename-" . $get_events[$k]['post_id'] . ".html";


    $post_releasedate = date($prefs_dateformat, $get_events[$k]['post_releasedate']);
    $post_releasedate_year = date('Y', $get_events[$k]['post_releasedate']);
    $post_releasedate_month = date('m', $get_events[$k]['post_releasedate']);
    $post_releasedate_day = date('d', $get_events[$k]['post_releasedate']);
    $post_releasedate_time = date($prefs_timeformat, $get_events[$k]['post_releasedate']);

    $get_events[$k]['event_releasedate'] = $post_releasedate;

    /* event dates */

    $get_events[$k]['event_start_day'] = date('d',$get_events[$k]['post_event_startdate']);
    $get_events[$k]['event_start_month'] = date('m',$get_events[$k]['post_event_startdate']);
    $get_events[$k]['event_start_month_text'] = $lang["m".$get_events[$k]['event_start_month']];
    $get_events[$k]['event_start_year'] = date('Y',$get_events[$k]['post_event_startdate']);
    $get_events[$k]['event_end_day'] = date('d',$get_events[$k]['post_event_enddate']);
    $get_events[$k]['event_end_month'] = date('m',$get_events[$k]['post_event_enddate']);
    $get_events[$k]['event_end_year'] = date('Y',$get_events[$k]['post_event_enddate']);

    /* entry date */
    $entrydate_year = date('Y', $get_events[$k]['post_date']);


    /* event categories */
    $post_categories = explode('<->', $get_events[$k]['post_categories']);
    $category = array();
    ...
    if ($get_events[$k]['post_votings'] == 2 || $get_events[$k]['post_votings'] == 3) {
        $get_events[$k]['show_voting'] = true;
        $voter_data = false;
        $voting_type = array("upv", "dnv");
        if ($get_events[$k]['post_votings'] == 2) {
            if ($_SESSION['user_nick'] == '') {
                $voter_data = false;
            } else {
                $voter_data = fc_check_user_legitimacy($get_events[$k]['post_id'], $_SESSION['user_nick'], $voting_type);
            }
        }

        if ($get_events[$k]['post_votings'] == 3) {
            if ($_SESSION['user_nick'] == '') {
                $voter_name = fc_generate_anonymous_voter();
                $voter_data = fc_check_user_legitimacy($get_events[$k]['post_id'], $voter_name, $voting_type);
            } else {
                $voter_data = fc_check_user_legitimacy($get_events[$k]['post_id'], $_SESSION['user_nick'], $voting_type);
            }
        }
        ...
        $votes = fc_get_voting_data('post', $get_events[$k]['post_id']);
