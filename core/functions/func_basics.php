...
function fc_search($query, $currentPage=1, $itemsPerPage=10) {
	...
	$sql = "SELECT page_url, page_title, page_description, page_images, snippet(pages, '<mark class=\"hi\">', '</mark>', '...', -1, -60) AS snipp, rank(matchinfo(pages)) AS score FROM pages WHERE pages MATCH :search ORDER BY score DESC LIMIT $startOffset, $endOffset;"; // LIMIT 0,10
